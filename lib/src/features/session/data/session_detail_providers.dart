...
class SessionDetailNotifier
    extends FamilyAsyncNotifier<SessionDetailState, int> {
  @override
  ...
  Future<SessionDetailState> _fetchData(int sessionId) async {
    ...
    final results = await Future.wait([
      ...
      repo.getAllLegacyPlayers().catchError((_) => <Player>[]),
    ]);
  ...
  Future<void> addPlayer({
    ...
    state = AsyncData(
      current.copyWith(participants: [...current.participants, tempSp]),
    );
  ...
  Future<void> addRebuy({
    ...
    try {
      await ref
          .read(sessionRepositoryProvider)
          .addRebuy(sessionPlayerId: sessionPlayerId, amountCents: amountCents);
    } catch (e) {
      // Revert on error
      state = AsyncData(current);
      rethrow;
  ...
  Future<void> deletePlayer({required int sessionPlayerId}) async {
    ...
    final updatedParticipants = current.participants
        .where((sp) => sp.id != sessionPlayerId)
        .toList();
    state = AsyncData(current.copyWith(participants: updatedParticipants));
    ...
    try {
      await ref
          .read(sessionRepositoryProvider)
          .deleteSessionPlayer(sessionPlayerId);
    } catch (e) {
      // Revert on error
      state = AsyncData(current);
      rethrow;
  ...
  Future<void> setSettlementMode({required String mode}) async {
    ...
    final updatedSession = Session(
      ...
      settlementMode: mode,
      bankerSessionPlayerId: mode == 'pairwise'
          ? null
          : current.session.bankerSessionPlayerId,
    );
    ...
    await ref
        .read(sessionRepositoryProvider)
        .setSettlementMode(sessionId: arg, mode: mode);
    if (mode == 'pairwise') {
      await ref
          .read(sessionRepositoryProvider)
          .setBanker(sessionId: arg, bankerSessionPlayerId: null);
    }
  }
