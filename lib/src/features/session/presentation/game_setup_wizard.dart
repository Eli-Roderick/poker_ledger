  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: asyncState.when(
        ...
        error: (_, __) =>
            const Center(child: Text('Legacy game could not be loaded.')),
        data: (data) {
              ...
              _SimpleProgressBar(
                ...
                onTap: (page) {
                  if (page < _currentPage ||
                      (page == 1 && hasEnoughPlayers) ||
                      (page == 2 && hasSettlementMode && hasEnoughPlayers)) {
                    _goToPage(page);
                  }
                },
              ),
        ...
        children: [
          ...
          OutlinedButton.icon(
            onPressed: availablePlayers.isEmpty
                ? null
                : () => _showAddPlayerDialog(context, ref, availablePlayers),
            icon: const Icon(Icons.person_add),
            label: Text(
              availablePlayers.isEmpty ? 'All players added' : 'Add Player',
            ),
          ),
          const SizedBox(height: 16),
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () async {
                                await ref
                                    .read(
                                      sessionDetailProvider(sessionId).notifier,
                                    )
                                    .deletePlayer(sessionPlayerId: sp.id!);
                              },
                            ),
                          ],
                        ),
          ...
          FilledButton(
            ...
            child: Text(
              hasEnoughPlayers
                  ? 'Next'
                  : 'Add ${2 - participants.length} more player${participants.length == 1 ? '' : 's'}',
            ),
          ),
        ],
      ),
                        ...
                        child: Padding(
                          ...
                          child: OutlinedButton(
                            onPressed: () => setDialogState(
                              () => buyInController.text = '$amount.00',
                            ),
          ...
          FilledButton(
            onPressed: () async {
              ...
              if (cents > 0) {
                await ref
                    .read(sessionDetailProvider(sessionId).notifier)
                    .addRebuy(sessionPlayerId: sp.id!, amountCents: cents);
              }
            },
...
class _SettlementModePage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_SettlementModePage> createState() =>
      _SettlementModePageState();
}
                  ...
                  onPressed: canContinue
                      ? () async {
                          ...
                          await ref
                              ...
                              .setSettlementMode(mode: _selectedMode!);
...
class _SummaryPageWrapper extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return asyncState.when(
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (_, __) =>
          const Center(child: Text('Legacy game data could not be loaded.')),
      data: (data) {
        final participants = data.participants;
        final cashedOutCount = participants
            .where((p) => p.cashOutCents != null)
            .length;
        final allCashedOut =
            participants.isNotEmpty && cashedOutCount == participants.length;
        ...
        final canFinalize = allCashedOut && difference == 0;
                  ...
                  Expanded(
                    child: FilledButton(
                      onPressed: canFinalize
                          ? () => _showFinalizeDialog(context, ref, data)
                          : null,
                      child: Text(
                        !allCashedOut
                            ? 'Enter $missingCashOuts more cash out${missingCashOuts == 1 ? '' : 's'}'
                            : difference != 0
                            ? 'Resolve ${NumberFormat.simpleCurrency().format(difference.abs() / 100)} difference'
                            : 'Finalize Game',
                      ),
                    ),
                  ),
...
class _WizardFinalizeDialog extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_WizardFinalizeDialog> createState() =>
      _WizardFinalizeDialogState();
}
